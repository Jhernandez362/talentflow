...
const workflowFiles = readdirSync(workflowDirectory)
  .filter((file) => /^(TF-ADMIN|TF-PUBLIC|TF-M4|TF-M5|TF-M6)-.*\.json$/.test(file))
  .sort();
...
if (errors.length > 0) {
  ...
  console.log(`OK: ${workflowFiles.length} workflows ADMIN/PUBLIC/M4/M5/M6 son importables y sus consultas estan parametrizadas.`);
}
